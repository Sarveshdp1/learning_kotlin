package c05_functions// lambda function is store inside a variable as an expression
// lambda return will exit the whole function not only it-self

// Kt01_Fundamentals.sum of 2 num
val sum1 = {a:Int, b:Int -> a+b}

/*
- Kt01_Fundamentals.sum of 3 num
- Int after -> is return type
- explicitly declared data type
 */
val sum2: (Int, Int, Int) -> Int = {a,b,c -> a+b+c}

// Kt01_Fundamentals.sum of array elements using lambda expression function
...
fun main() {
    ...
    // Kt01_Fundamentals.sum of nums
    println(sum1.invoke(n1, n2))
    println(sum2.invoke(n1,n2,n3))

    // Kt01_Fundamentals.sum of array element
    val arr:Array<Int> = arrayOf(1,2,3,4)
    println(sumOfArray.invoke(arr))

    // lambda as class extension
    val result = "Sarvesh's kt05_functions.age iss ".lambdaAsClass(20)
    println(result)
    ...
    print("hello word") // on the 3 the return exit the Kt01_Fundamentals.kt02_DataTypes.kt03_conditional_expression.kt04_loops.kt05_functions.kt06_collection.kt07_object_oriented_programming.main function so the print function never runs so it's important to use labeled return
}