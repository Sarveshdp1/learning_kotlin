...
// normal, anonymous, lambda, higher-order all these functions are treated as objects so compiler creates memory for individual which can leads to performance drop
...
// so using the inline function we are only using the memory of the function which called the other function like a main function and not creating individual memory for the functions we called
...
// calling facto lambda function and copying the code to main function for execution
...
// if we don't want some parameter lambda not to pass as inline we can use noinline keyword like crossinline keyword
...
fun main() {
    print("Enter Number: ")
    val n = readln().toInt()
    higherOrderFacto(n,facto) // higher order inline function copy the main function and execute it here
    ...
    // directly passing the lambda function via parameter without creating it outside the main function
    ...
    // next statement will not execute because the previous call which has lambda return, returned/exited the main function
    // this happened because the lambda return returns the function in which the lambda is executed
    // when the nonLocal function has called which is an inline function it copied the body of that function and executed the code in the main function mean the function who called the inline function
    // so basically it copied the exact lamba and created as local lambda {print("hello"); return}, {print("world")} when executed in main function the return statement retuned in the main function as well and program ended
